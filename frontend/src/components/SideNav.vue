<template>
      ...
      <ul class="nav text-black fw-semibold flex-column">
        ...
        <li v-if="userRole === 'manager'" class="mb-2">
          <router-link to="/kpis" class="nav-link" :class="{ active: isActive('/kpis') }">
            <div class="d-flex align-items-center gap-3">
              <svg class="w-10" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M3 17V21H7V17H3Z" fill="#2ca9bc"/>
                <path d="M10 13V21H14V13H10Z" fill="#000"/>
                <path d="M17 8V21H21V8H17Z" fill="#2ca9bc"/>
              </svg>
  ...
  <div class="d-md-none mobile-sidenav p-2 bg-light border-bottom">
    ...
    <ul v-if="open" class="nav flex-column mt-2">
      ...
      <li class="nav-item"><a class="nav-link" @click.prevent="logout" style="cursor:pointer;">Logout</a></li>
    </ul>
  </div>
</template>
...
// normalize and check active state (case-insensitive, prefix match for admin routes)
function isActive(path) {
  if (!route.path) return false
  const rp = route.path.toLowerCase()
  const pp = path.toLowerCase()
